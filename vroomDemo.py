...
def deltaVec((x,y), tdelt):
	return (x*tdelt, y*tdelt)

def crossings(rotatedVertices0, rotatedVertices1, p0, p1, tree):
	rotatedVertexPairs = map(None, rotatedVertices0, rotatedVertices1)
	xings = []
...
class car:
	...
	def processFinishLineCrossings(self, i, xings):
		lapinc = 0
		for (x0, x1) in map(None, xings[i][:-1], xings[i][1:]):
...
def main(objects, player):
	...
	if not globals.tree:
		dumpFile = open("bsp/"+game.settings.track[0]+"_bsp", 'r')
		globals.tree = cPickle.load(dumpFile)
		dumpFile.close()
...
def run():
	...
	while 1:
		...
		main(objects, player)
		player.lastChan.stop()
		if sounds:
			stopSounds()
		scores2.showScores(scores2.scores, "local")
		try:
			scores2.saveScores()
		except error.vroomException, val:
			mso = error.errOut()
			print >> mso, val
			pygame.display.flip()
...
try:
	...
	class initialize:
		...
		if dinfo.bitsize<24:
			...
			dinfo = pygame.display.Info()
			text = "Current display mode is "+`dinfo.bitsize`+" bits."
			size = font.size(text)
