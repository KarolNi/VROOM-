...
def registerScore(time):
	new = [string.rjust(fpformat.fix(float(time)/1000, 2), 7), scoreEntry, 5]
...
def saveScores():
	...
	for score in scores[:20]:
		print >> scoreFile, `score[2]`+":"+score[0]+" "+score[1];
	scoreFile.close()
...
def showScores(scores, type, page=0):
	...
	for score in scores[page*20:(page*20)+20]:
		...
		font.blit(`idx+page*20`+".", blackScreen, (tab-7+x1, offset))
	...
	while 1:
		...
		screen.blit(blackScreen, (0,0))
		if page>0:
			textRect = font.center("page "+`page+1`, blackScreen, 37.5)
